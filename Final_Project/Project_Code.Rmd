---
title: "POLI 178 - Final Project R Code"
author: "Andrew Acosta, Daniel Barquero, Raymond Gazzo, Yan Graf, Ryan Liao, Jeffrey Liu, Raphael Reaport, Calli Tullis, and Crystal Zhan"
date: "2022-12-04"
...
We want to subset for years after 2000.
...
We want to add country codes to ensure matches across datasets. We will manually do this if needed.
...
# save df to save computation time for future use
...
# histogram of types of financial diplomacy
...
# histogram of the intent for financial diplomacy
...
# histogram of the region receiving financial diplomacy
...
It seems that there are a few countries that have gotten the most from China. We will do further analysis to name which ones.
...
The top 10 countries are Angola, Cambodia, Myanmar, Sudan, Pakistan, Indonesia, the Democratic Republic of the Congo, Laos, Ethiopia, and Tanzania.
...
# tile the African subregions together
...
Now we need to deal with missingness in data. We use one rule of thumb: if the GDP doesn't exist, data are likely insufficient for the model.
...
Missing data for arms imports, arms exports, imports, and exports can be considered $0. We assume missing data for resources means 0 of GDP is accounted by natural resources. We also assume that because missing polity indicates foerign intervention, the regime is neither democratic nor autocratic (0 for polity).
...
However, arms data is too militarily based and pretty sparse. IGO alignment is much too sparse and difficult to interpolate. Financial diplomacy only lasts to 2017, and it is hard to make future predictions. We will remove all of these from our dataframe.

```{r}
df <- df %>% select(-c("Arms.Imports","Arms.Exports","IGO.Alignment","Fin.Diplomacy"))
```

We need to prepare data for two models. One for predicting FDI (2005-2020), and one for UN agreement (2000-2020).
...
We see that the random forest model performs the best for the economic and diplomatic model, as well as for both the $R^2$ and RMSE metrics. We will use the random forest model on our testing data.
...
The latest year we have all data for is 2020. We will use this set of data to predict into the future.
